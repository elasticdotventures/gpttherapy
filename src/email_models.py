...
from .datetime_utils import utc_now_iso, datetime_to_instant
...
class EmailContent(BaseModel):
    ...
    contains_response: bool = Field(description="Whether email contains meaningful response")
    
    @field_validator('word_count')
    @classmethod
    def validate_word_count(cls, v: int, values) -> int:
    ...
    @classmethod
    def validate_contains_response(cls, v: bool, values) -> bool:
...
class ParsedEmail(BaseModel):
    ...
    to_addresses: List[EmailStr] = Field(min_length=1, description="Recipient email addresses")
    cc_addresses: List[EmailStr] = Field(default_factory=list, description="CC email addresses")
    bcc_addresses: List[EmailStr] = Field(default_factory=list, description="BCC email addresses")
    
    # Email metadata
    subject: str = Field(max_length=998, description="Email subject line")  # RFC 5322 limit
    message_id: str = Field(description="Unique message identifier")
    timestamp: datetime = Field(description="Email timestamp")
    
    # Content
    body_text: str = Field(description="Plain text body")
    body_html: Optional[str] = Field(default=None, description="HTML body (if present)")
    ...
    reply_to_message_id: Optional[str] = Field(default=None, description="Message ID being replied to")
    thread_id: Optional[str] = Field(default=None, description="Thread identifier")
    
    # Processing metadata
    spam_score: int = Field(default=0, ge=0, le=10, description="Spam likelihood score (0-10)")
    is_automated: bool = Field(default=False, description="Whether email appears automated")
    processing_timestamp: datetime = Field(default_factory=lambda: datetime_to_instant(datetime.now()).py_datetime())
    
    # Game/therapy specific content
    extracted_content: Optional[EmailContent] = Field(default=None, description="Extracted game content")
    ...
    def validate_body_text(cls, v: str) -> str:
        ...
        max_length = settings.MAX_EMAIL_BODY_LENGTH if hasattr(settings, 'MAX_EMAIL_BODY_LENGTH') else 50000
        if len(v) > max_length:
    ...
    def calculate_spam_score(self) -> int:
        ...
        # Excessive capitals
        caps_ratio = sum(1 for c in self.body_text if c.isupper()) / max(len(self.body_text), 1)
        if caps_ratio > 0.5:
            score += 3
        ...
        spam_indicators = [
            ...
            'winner', 'congratulations', 'urgent', 'immediate action'
        ]
