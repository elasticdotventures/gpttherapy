...
def process_ses_email(record: dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.error(
            f"Error processing SES record: {str(e)}", exc_info=True, extra=error_context
        )
        raise
...
def process_session_turn(
    session_info: dict[str, Any], mail: dict[str, Any], receipt: dict[str, Any]
) -> None:
    """
...
Session: {session_id}"""
            ...
            send_response_email(
                ...
                subject="Turn Received - Waiting for Others",
                body=ack_message,
            )

        logger.info(
            f"Successfully processed turn {game_state['current_turn']} for session {session_id}"
        )
        ...
        error_context = {
            ...
            "turn_data": (
                str(turn_data)[:200] if "turn_data" in locals() else "unavailable"
            ),
        }
        logger.error(
...
def initialize_new_session(mail: dict[str, Any], receipt: dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.error(
            f"Error initializing session for {player_email}: {str(e)}",
            exc_info=True,
            extra=error_context,
        )
