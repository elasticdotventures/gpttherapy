...
def handle_session_pause(session_info: Dict[str, Any], timeout_result: Dict[str, Any]) -> None:
    ...
    try:
        storage.archive_email(session_id, {
            'type': 'system_event',
...
Session: {session_id}"""
            ...
            send_email(player_email, reminder_subject, reminder_body, session_id, game_type)
            logger.info(f"Sent timeout reminder to {player_email} for session {session_id}")
            
        except Exception as e:
            logger.error(f"Failed to send timeout reminder to {player_email}: {e}")

def send_continuation_notifications(session_info: Dict[str, Any], timeout_result: Dict[str, Any]) -> None:
    ...
    try:
        ...
        for player_email in active_players:
            subject = f"[Adventure Continues] Turn {timeout_result['current_turn']} Complete"
...
Session: {session_id}"""
            ...
            send_email(player_email, subject, body, session_id, session_info['game_type'])
            
    except Exception as e:
        logger.error(f"Failed to send continuation notifications: {e}")

def send_email(to_address: str, subject: str, body: str, session_id: str, game_type: str) -> None:
                ...
                if cleanup_old and session.get('status') == 'completed':
                    ...
                    if created_at:
                        ...
                        if age > timedelta(days=30) and not dry_run:
                            ...
                            logger.info(f"Session {session_id} marked for archival (age: {age.days} days)")
                            health_results['cleaned_up'].append(session_id)
                            
            except Exception as e:
                logger.error(f"Error checking health for session {session.get('session_id')}: {e}")
    ...
    try:
        ...
        for session in active_sessions[:max_sessions]:
            try:
                ...
                logger.error(f"Error backing up session {session.get('session_id')}: {e}")
                backup_results['errors'].append({
                    'session_id': session.get('session_id'),
                    'error': str(e)
                })
...
def process_reminder_sending(options: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        cutoff_time = datetime.now(timezone.utc) - timedelta(hours=reminder_threshold)
        ...
        for session in active_sessions[:max_sessions]:
            try:
                ...
                reminder_results['reminders_sent'].append({
                    'session_id': session_id,
                    'reminded_players': waiting_for
                })
                
            except Exception as e:
                logger.error(f"Error processing reminders for session {session.get('session_id')}: {e}")
...
Session: {session_id}"""
            ...
            logger.info(f"Sent gentle reminder to {player_email} for session {session_id}")
