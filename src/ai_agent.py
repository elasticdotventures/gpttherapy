...
class AIAgent:
    ...
    def generate_response(self, 
        ...
        try:
            ...
            logger.info(f"Generated response for {game_type} session {session_context.get('session_id')}")
            return response
...
"""
    ...
    def _build_user_prompt(self, 
        ...
        prompt_parts.append(f"From: {session_context.get('current_player', 'Unknown player')}")
        prompt_parts.append(f"Message: {player_input}")
        prompt_parts.append("")
        
        # Add instructions
        prompt_parts.append("## Instructions")
        prompt_parts.append("Please generate an appropriate response based on your role and the session context. ")
        
        if session_context.get('game_type') == 'dungeon':
            prompt_parts.append("Advance the adventure story, respond to the player's actions, and provide engaging narrative descriptions.")
        elif session_context.get('game_type') == 'intimacy':
            prompt_parts.append("Provide therapeutic guidance, validate emotions, and suggest constructive exercises or reflections.")
...
Dr. Alex Chen, LMFT"""
        ...
        return fallbacks.get(game_type, "Thank you for your message. I'll respond shortly.")
    
    def generate_initialization_response(self, 
                                       game_type: str, 
                                       player_email: str,
                                       session_id: str) -> str:
...
def generate_ai_response(game_type: str, 
                        session_context: Dict[str, Any],
                        player_input: str,
                        turn_history: List[Dict[str, Any]] = None) -> str:
    """
    ...
    """
    agent = get_ai_agent()
    return agent.generate_response(game_type, session_context, player_input, turn_history)