...
def should_exclude(path: Path, project_root: Path) -> bool:
    ...
    exclude_patterns = {
        ...
        ".ruff_cache", "node_modules", ".coverage",
...
def create_tar_archive(source_dir: Path, output_file: Path) -> None:
    ...
    with tarfile.open(output_file, "w:gz") as tar:
        for root, dirs, files in os.walk(source_dir):
            ...
            dirs[:] = [d for d in dirs if not should_exclude(Path(root) / d, source_dir)]
...
def main():
    ...
    parser.add_argument(
        "--include-git",
        action="store_true",
        help="Include .git directory in archive"
    )
