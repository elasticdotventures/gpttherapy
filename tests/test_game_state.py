...
from src.game_state import (
    GameStateManager, GameStateType, CharacterState, WorldState, 
    NarrativeState, TherapyState, MissionState,
    get_game_state_manager, save_character_state, load_character_state, update_world_state
)
...
class TestDataClasses:
    ...
    def test_therapy_state_creation(self):
...
class TestGameStateManager:
    ...
    def test_init(self, game_state_manager):
        """Test GameStateManager initialization."""
        assert game_state_manager.storage is not None
    
    def test_save_game_state_dict(self, game_state_manager, mock_storage):
        ...
        assert save_data['state_document']['state_type'] == GameStateType.WORLD_STATE.value
        assert save_data['state_document']['state_data'] == state_data
    
    def test_save_game_state_dataclass(self, game_state_manager, mock_storage):
    ...
    def test_load_game_state_success(self, game_state_manager, mock_storage):
        ...
        result = game_state_manager.load_game_state('session-123', GameStateType.WORLD_STATE)
        
        assert result == {'current_location': 'castle'}
        mock_storage.load_game_state.assert_called_once_with('session-123')
    
    def test_load_game_state_not_found(self, game_state_manager, mock_storage):
        """Test loading game state when not found."""
        mock_storage.load_game_state.return_value = None
        
        result = game_state_manager.load_game_state('nonexistent', GameStateType.WORLD_STATE)
        
        assert result is None
    
    def test_create_character_state(self, game_state_manager, mock_storage, sample_character_data):
    ...
    def test_update_character_state(self, game_state_manager, mock_storage):
        ...
        mock_storage.load_game_state.return_value = {
    ...
    def test_update_character_state_not_found(self, game_state_manager, mock_storage):
    ...
    def test_create_world_state(self, game_state_manager, mock_storage):
    ...
    def test_update_world_state_existing(self, game_state_manager, mock_storage, sample_world_data):
    ...
    def test_update_world_state_new(self, game_state_manager, mock_storage):
    ...
    def test_create_therapy_state(self, game_state_manager, mock_storage):
    ...
    def test_update_therapy_progress(self, game_state_manager, mock_storage):
        ...
        result = game_state_manager.update_therapy_progress('session-123', progress_update)
    ...
    def test_get_session_summary(self, game_state_manager, mock_storage):
        ...
        def mock_load_game_state(session_id, state_type):
            if state_type == GameStateType.WORLD_STATE:
                return {'current_location': 'castle', 'updated_at': '2023-01-01T12:00:00Z'}
            elif state_type == GameStateType.THERAPY_STATE:
                return {'current_phase': 'communication', 'updated_at': '2023-01-01T13:00:00Z'}
            else:
                return None
    ...
    def test_backup_session_state(self, game_state_manager, mock_storage):
        """Test backing up session state."""
        # Mock session summary
        game_state_manager.get_session_summary = Mock(return_value={
...
class TestUtilityFunctions:
    ...
    def test_get_game_state_manager(self):
    ...
    def test_save_character_state_convenience(self, mock_storage):
        ...
        with patch('src.game_state.GameStateManager') as mock_manager_class:
            ...
            result = save_character_state('session-123', 'player@example.com', {'name': 'Test'})
            
            assert result is True
            mock_instance.create_character_state.assert_called_once()
    
    def test_load_character_state_convenience(self, mock_storage):
    ...
    def test_update_world_state_convenience(self, mock_storage):
...
class TestErrorHandling:
    ...
    def test_save_game_state_storage_error(self, game_state_manager, mock_storage):
    ...
    def test_load_game_state_storage_error(self, game_state_manager, mock_storage):
    ...
    def test_get_session_summary_error(self, game_state_manager, mock_storage):
        """Test handling errors during session summary."""
        # Mock a more fundamental error that would cause the whole operation to fail
        with patch.object(game_state_manager, 'load_game_state', side_effect=Exception("Load error")):
