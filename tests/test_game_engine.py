...
from src.game_engine import GameEngine, TurnState, SessionState, get_game_engine, process_turn
...
class TestGameEngine:
    ...
    def test_init(self, game_engine):
    ...
    def test_process_player_turn_session_not_found(self, game_engine, mock_storage):
    ...
    def test_process_player_turn_player_not_in_session(self, game_engine, mock_storage, sample_session):
    ...
    def test_check_turn_completion_intimacy(self, game_engine, mock_storage, sample_intimacy_session):
        ...
        mock_storage.get_session_turns.return_value = [
            {
                'turn_number': 2,
                'player_email': 'partner1@example.com'
            },
            {
                'turn_number': 2,
                'player_email': 'partner2@example.com'
            }
        ]
        
        result = game_engine._check_turn_completion('therapy-session-456', 2, sample_intimacy_session)
        assert result is True
    
    def test_check_turn_completion_intimacy_incomplete(self, game_engine, mock_storage, sample_intimacy_session):
    ...
    def test_advance_turn(self, game_engine, mock_storage, sample_session):
    ...
    def test_check_turn_timeouts(self, game_engine, mock_storage):
    ...
    def test_add_player_to_session_success(self, game_engine, mock_storage):
    ...
    def test_add_player_to_session_full(self, game_engine, mock_storage):
        """Test adding player to full session."""
        full_session = {
            'session_id': 'full-123',
            'game_type': 'dungeon',
            'players': ['p1@example.com', 'p2@example.com', 'p3@example.com', 'p4@example.com']
        }
    ...
    def test_add_player_already_exists(self, game_engine, mock_storage, sample_session):
        """Test adding player who's already in session."""
        mock_storage.get_session.return_value = sample_session
        
        result = game_engine.add_player_to_session('test-session-123', 'player1@example.com')
        
        assert 'error' in result
        assert 'already in session' in result['error']
    
    def test_get_turn_summary(self, game_engine, mock_storage):
        ...
        assert result['submissions']['player1@example.com']['email_content']['body'] == 'I attack the dragon'
    
    def test_get_turn_summary_latest(self, game_engine, mock_storage):
...
class TestErrorHandling:
    ...
    def test_process_turn_storage_error(self, game_engine, mock_storage, sample_session):
    ...
    def test_check_timeouts_storage_error(self, game_engine, mock_storage):
    ...
    def test_handle_timeout_session_not_found(self, game_engine, mock_storage):
