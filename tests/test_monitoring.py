...
from src.monitoring import (
    MetricsCollector, HealthMonitor, TimerContext, Metric, HealthCheck,
    MetricType, timing_decorator, error_tracking_decorator,
    get_system_metrics, create_monitoring_dashboard,
    track_email_processed, track_turn_completed, track_ai_response_time,
    track_session_created, track_session_completed
)
...
class TestMetricsCollector:
    ...
    def test_unit_detection(self, metrics_collector):
        """Test automatic unit detection for metrics."""
        assert metrics_collector._get_unit_for_metric('response.duration') == 'Milliseconds'
...
class TestTimerContext:
    ...
    def test_timer_context_direct(self):
...
class TestHealthMonitor:
    ...
    def test_database_health_check_success(self, health_monitor):
        """Test successful database health check."""
        health_monitor.storage.get_active_sessions.return_value = ['session1', 'session2']
    ...
    def test_database_health_check_failure(self, health_monitor):
        """Test database health check failure."""
        health_monitor.storage.get_active_sessions.side_effect = Exception("Connection failed")
    ...
    def test_storage_health_check_success(self, mock_boto3, health_monitor):
    ...
    @patch('src.monitoring.boto3.client')
    def test_storage_health_check_failure(self, mock_boto3, health_monitor):
    ...
    @patch('src.monitoring.boto3.client')
    def test_ai_service_health_check_success(self, mock_boto3, health_monitor):
    ...
    @patch('src.monitoring.boto3.client')
    def test_ai_service_health_check_failure(self, mock_boto3, health_monitor):
    ...
    def test_run_all_health_checks_healthy(self, health_monitor):
        """Test running all health checks when all are healthy."""
        with patch.object(health_monitor, 'check_database_health', 
                          return_value={'status': 'healthy', 'message': 'OK', 'response_time_ms': 10}):
            with patch.object(health_monitor, 'check_storage_health',
                              return_value={'status': 'healthy', 'message': 'OK', 'response_time_ms': 20}):
                with patch.object(health_monitor, 'check_ai_service_health',
                                  return_value={'status': 'healthy', 'message': 'OK', 'response_time_ms': 30}):
    ...
    def test_run_all_health_checks_unhealthy(self, health_monitor):
...
class TestDecorators:
    ...
    def test_error_tracking_decorator_success(self):
    ...
    def test_error_tracking_decorator_error(self):
...
class TestSystemMetrics:
    ...
    def test_get_system_metrics(self, mock_getpid, mock_psutil):
        """Test system metrics collection."""
...
class TestConvenienceFunctions:
    ...
    def test_track_email_processed(self):
        ...
        with patch('src.monitoring.metrics') as mock_metrics:
            ...
            mock_metrics.counter.assert_called_once_with(
                'emails.processed',
                tags={'game_type': 'dungeon', 'status': 'success'}
            )
    
    def test_track_turn_completed(self):
    ...
    def test_track_ai_response_time(self):
    ...
    def test_track_session_created(self):
        ...
        with patch('src.monitoring.metrics') as mock_metrics:
            ...
            mock_metrics.counter.assert_called_once_with(
                'sessions.created',
                tags={'game_type': 'intimacy'}
            )
    
    def test_track_session_completed(self):
...
class TestMonitoringDashboard:
    ...
    def test_create_monitoring_dashboard(self, mock_metrics, mock_system_metrics, mock_health_monitor):
        ...
        mock_system_metrics.return_value = {
            'system': {'cpu_percent': 25.0, 'memory_percent': 60.0, 'disk_percent': 40.0},
