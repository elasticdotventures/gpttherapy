...
from src.ai_agent import AIAgent, generate_ai_response, get_ai_agent
...
def sample_bedrock_response():
    ...
    return {"body": Mock(), "contentType": "application/json"}
...
class TestAIAgent:
    ...
    def test_init(self, ai_agent) -> None:
    ...
    def test_load_agent_configs(self, mock_bedrock_client) -> None:
    ...
    def test_generate_response_success(
        self, ai_agent, mock_bedrock_client, session_context, turn_history
    ) -> None:
    ...
    def test_generate_response_bedrock_error(
        self, ai_agent, mock_bedrock_client, session_context
    ) -> None:
    ...
    def test_build_system_prompt(self, ai_agent, session_context) -> None:
    ...
    def test_build_user_prompt(self, ai_agent, session_context, turn_history) -> None:
    ...
    def test_build_user_prompt_intimacy(self, ai_agent, session_context) -> None:
    ...
    def test_generate_initialization_response_with_template(self, ai_agent) -> None:
...
Ready to begin?"""
    ...
    def test_generate_initialization_response_no_template(self, ai_agent) -> None:
    ...
    def test_fallback_responses(self, ai_agent) -> None:
    ...
    def test_call_bedrock_success(self, ai_agent, mock_bedrock_client) -> None:
    ...
    def test_call_bedrock_unexpected_response_format(
        self, ai_agent, mock_bedrock_client
    ) -> None:
...
class TestUtilityFunctions:
    ...
    def test_get_ai_agent(self, mock_bedrock_client) -> None:
        """Test get_ai_agent function."""
        with patch("pathlib.Path.exists", return_value=False):
            agent = get_ai_agent()
            assert isinstance(agent, AIAgent)

    def test_generate_ai_response(self, mock_bedrock_client, session_context) -> None:
        ...
        with (
            ...
            patch.object(
                AIAgent, "generate_response", return_value=mock_response
            ) as mock_generate,
        ):
...
class TestErrorHandling:
    ...
    def test_missing_agent_config(self, mock_bedrock_client) -> None:
        """Test handling missing agent configuration files."""
        with patch("pathlib.Path.exists", return_value=False):
            agent = AIAgent()
            assert len(agent.agent_configs) == 0

    def test_file_read_error(self, mock_bedrock_client) -> None:
    ...
    def test_bedrock_json_parse_error(self, ai_agent, mock_bedrock_client) -> None:
        """Test handling JSON parse errors from Bedrock."""
        mock_bedrock_client.invoke_model.return_value = {
            "body": Mock(**{"read.return_value": b"invalid json"})
        }
